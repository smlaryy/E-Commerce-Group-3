                        ...
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <div class="text-sm font-semibold text-gray-900">
                                    #{{ $order->id }} • {{ $order->buyer->name ?? 'Pembeli' }}
                                </div>
                            ...
                            <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium {{ $badgeClass }}">
                                {{ ucfirst($status) }}
                            </span>
                        </div>

                        <div class="mt-2 text-xs text-gray-600 flex justify-between">
                            <span>Total: <span class="font-semibold text-gray-900">
                                    Rp {{ number_format($order->total_price ?? 0, 0, ',', '.') }}
                                </span></span>
                            <span>
                                Item:
                                {{ optional($order->details)->sum('qty') ?? '-' }}
                            </span>
                    ...
                    <table class="min-w-full divide-y divide-gray-100 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                'paid' => ['label' => 'Sudah Dibayar', 'class' => 'bg-green-50 text-green-700'],
                                'shipped' => ['label' => 'Dikirim', 'class' => 'bg-blue-50 text-blue-700'],
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($status)
                                    <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium {{ $status['class'] }}">
                                        {{ $status['label'] }}
                                    </span>
                                    @else
